...
const ProfileGallery = (props) => {
  ...
  const fitUser = (res1, res2, res3, res4, res5, res6, res7, res8, res9, res10, res11, res12) => {
    if (user.res1 == res1) setFit(fit + 1)
    if (user.res2 == res2) setFit(fit + 1)
    if (user.res3 == res3) setFit(fit + 1)
    if (user.res4 == res4) setFit(fit + 1)
    if (user.res5 == res5) setFit(fit + 1)
    if (user.res6 == res6) setFit(fit + 1)
    if (user.res7 == res7) setFit(fit + 1)
    if (user.res8 == res8) setFit(fit + 1)
    if (user.res9 == res9) setFit(fit + 1)
    if (user.res10 == res10) setFit(fit + 1)
    if (user.res11 == res11) setFit(fit + 1)
    if (user.res12 == res12) setFit(fit + 1)
  }
            ...
            <GridListTileBar
              ...
              actionIcon={
                <IconButton
                  ...
                  onClick={() => fitUser(image.res1, image.res2, image.res3, image.res4, image.res5, image.res6, image.res7, image.res8, image.res9, image.res10, image.res11, image.res12)}
                >
                  dopasowanie {Math.floor(Math.random() * 100)}%
                </IconButton>
              }
            />
          ) : null}
